...
\begin{definition}[Rule and match]
\label{def:rulematch}
Every event $e$ of a history has a \textbf{rule}, which is one of three inclusions of finite simplicial sets.
The big bang has rule $\varnothing \hookrightarrow \Delta^0$.
A sprout has rule $d^1 : \Delta^0 \hookrightarrow \Delta^1$.
A fill has rule $\Lambda^n_k \hookrightarrow \Delta^n$ for some $0 < k < n$.
The left- and right-hand sides of the rule are written $L_e$ and $R_e$, following double-pushout rewriting \citep{ehrig2006dpo}.
The \textbf{match} of $e$ is a map $m_e : L_e \to X_{<e}$ from the left-hand side of its rule into the state of the events strictly preceding $e$, specifying the spot at which the rule is applied.
The match of the big bang is the unique map out of the empty simplicial set.
\end{definition}
...
\begin{enumerate}
  ...
  \item \emph{Inductive step.}
        ...
        Let $L_b \hookrightarrow R_b$ be the rule of $b$ and $m_b : L_b \to X_{<b}$ its match, as in Definition~\ref{def:rulematch}.
