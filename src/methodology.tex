...
The match is a map $m_e : F \to X_{<e}$ from the find pattern into the state of the events strictly preceding $e$ (Definition~\ref{def:state}), and apply is the pushout of the rule along the match.
...
\begin{itemize}
  ...
  \item \textbf{A fill.}
    ...
    Match: the chosen horn, a map $\Lambda^n_k \to X_{<e}$.
...
\end{tikzcd}
\]
Pushouts of monomorphisms in $\sset$ are monomorphisms, so the image of $m_b$ persists in $X \cup_{F_a} A_a$ and $b$ can be applied after $a$, and symmetrically.
