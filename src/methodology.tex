...
\label{def:history}
A \textbf{history} $\mathcal{H}$ is a partially ordered set of events with a least element $\bot$, the big bang, in which every other event $e$ is a sprout or a fill applied at a specified spot of the simplicial set built by the events strictly preceding $e$ (the state $X_{<e}$ of Definition~\ref{def:state}), subject to three conditions:
...
\begin{enumerate}
  ...
  \item \emph{Inductive step.}
        ...
        The event $b$ is a sprout, a fill, or the big bang, and its rule is written as an inclusion $L_b \hookrightarrow R_b$ of finite simplicial sets. For a sprout $L_b = \Delta^0$ and $R_b = \Delta^1$, for a fill $L_b = \Lambda^n_k$ and $R_b = \Delta^n$, and for the big bang $L_b = \varnothing$ and $R_b = \Delta^0$.
        The spot of $b$ is recorded in the history as a map $m_b : L_b \to X_{<b}$, called the \emph{match} of $b$, with $X_{<b}$ as in Definition~\ref{def:state}.
        The causal past of $b$ is a downset contained in $D'$, and building $X_{D'}$ along a linear extension that lists this past first passes through $X_{<b}$.
        Each attachment is a pushout of a monomorphism and hence a monomorphism, so $X_{<b}$ includes into $X_{D'}$, and we also write $m_b$ for the composite $L_b \to X_{<b} \hookrightarrow X_{D'}$.
